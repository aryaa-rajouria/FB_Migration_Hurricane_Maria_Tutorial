...
A few examples of instances in which the Facebook Ads Manager has previously been used include: 
  ...
  *[To create global COVID-19 Surveys to aid in policy-making](https://csde.washington.edu/seminar/partnering-with-a-global-platform-toinform-research-and-public-policy-making-what-needs-to-be-in-place-to-make-a-global-covid-19-survey-work/) which was presented at the CSDE seminar series and is available on YouTube [here](https://www.youtube.com/watch?v=t_hvQEYK9Xg).
...
library(viridis)         # for pretty graphs
...
We ran some descriptives and created some visualizations to better understand the clean data. Each new 'wave' of data is collected on the first of the month, every couple of months. As is evident from the table below, the number of migrants (called expat_population) is similar from Wave 1 to Wave 5. There is a decrease from Wave 5 to Wave 6. For the last wave (Wave 7) the migrant count remains stagnant. 

```{r}
#looking at migrant count by date: helps us see what 'dates' are what 'waves'
mig_count <- fb %>% group_by(wave, date) %>% summarise(n = n_distinct(expat_population))
kable(mig_count, 
...
mig_flow <- fb %>% select(origin, state, expat_population)
...
Then, we see that of all the migrants in the Facebook Ads Manager data, there were 258 females and 257 males. That is almost exactly the same. 

```{r}
#looking at how many people who migrated were male vs female 
sex_comparison <- fb %>% group_by(sex) %>% summarise(n = n_distinct(expat_population))

rownames(sex_comparison) <- c("female", "male")
kable(sex_comparison, 
      format.args=list(big.mark=','), col=c("sex", "count")) %>%
  kable_paper()

#sex comparison visualization: not very helpful, both look the same here

#ggplot(sex_comparison, aes(x=sex, y=n)) + 
   #geom_bar(position="dodge", stat="identity")+
    #labs( x = "sex", y = "count") +
    #scale_x_discrete(name = "sex", limits=c("female", "male")) +
    #col="#69b3a2"
```

When the data is disaggregated as per the different waves, we find that there are differences in the migration experiences of males and females. In general, there are slightly more male migrants than female migrants. This difference is a little more accentuated in Wave 1, Wave 3, Wave 5 and Wave 6. 


```{r}
#looking at how many people who migrated were male vs female according to wave 
sex_wave_comparison <- fb %>% group_by(wave, sex) %>% summarise(n = n_distinct(expat_population))
...
Now, we explore age. As per the table and graph below, of all the migrants in the Facebook data, there were more that migrants in their mid-20s and mid-30s compared to younger (teenagers) or older (past 50) populations. 

```{r}
#looking at how many people who migrated were of what age group generally 
age_comparison <- fb %>% group_by(age_group) %>% summarise(n = n_distinct(expat_population))

kable(age_comparison, 
      format.args=list(big.mark=','), col=c("age", "count")) %>%
  kable_paper()

ggplot(age_comparison, aes(x=age_group, y=n))+
...
When disaggregated by waves, the pattern differs. [This graph looks strange, check back here: it's saying that there are more migrants that are 50 years + which is not at all true]
```{r}
#looking at how many people who migrated were of what age according to wave
age_wave_comparison <- fb %>% group_by(wave, age_group) %>% summarise(n = n_distinct(expat_population))

ggplot(age_wave_comparison, aes(x=wave, y=n, fill=age_group)) +
  geom_bar(position="dodge", stat="identity") +
    labs( x = "wave", y = "count", fill = "age") +
    coord_flip()+
    scale_x_discrete(name = "wave", limits=c(1,2,3,4,5,6,7)) 
```
## **National FB population estimates**

Alexander et al use FB data to estimate US population. From their estimations, there seems to be a general decrease in the US population from Wave 1 to Wave 7, with an particular decrease in Wave 5. These estimations are an undercount. In 2017, the US population was around 325 million according to the US Census Bureau [American Community Survey](https://data.census.gov/cedsci/table?tid=ACSDP1Y2017.DP05) and [Population Estimates Program](https://data.census.gov/cedsci/table?q=PEPANNRES%3A%20ANNUAL%20ESTIMATES%20OF%20THE%20RESIDENT%20POPULATION%3A%20APRIL%201,%202010%20TO%20JULY%201,%202019%20-%20FOR%20FULL%20ESTIMATES%20DETAIL,%20VISIT%20https%3A%2F%2Fwww.census.gov%2Fprograms-surveys%2Fpopest.html&tid=PEPPOP2019.PEPANNRES).

Note: In the code, Alexander et al filter by Mexico, but it doesn't necessarily matter: all countries have the same total population.
```{r}
...
ggplot(data=total_fb_pop, aes(x=wave, y=fb)) +
  geom_line()+
  geom_point()+
  labs(title="Estimating US Population Using FB data",x="Waves", y = "Population Estimate [Count]") +
  scale_x_discrete(name = "wave", limits=c(1,2,3,4,5,6,7)) +
  ylim(100000000, 184755000)
...
In the FB data, we find a large number of Mexican migrants in the United States. This is true for all 7 wave (from January 2017 to July 2018). The number of Mexican migrants is significantly larger than the estimated migrant population from the other 27 origin countries. There is a marked a marked difference in the number of migrants from Puerto Rico between Waves 3 and 4, 4 and 5, and beyond Wave 5.

Note: There is a difference is counts between Waves 5 and 6 (and a extreme similarity between Waves 6 and 7). This potentially illustrates that FB data isn't always stable: it can suddenly change. Alexander et al note that a limitation is the opaqueness of the Facebook algorithm or shift in privacy policies. For example, it is interesting to note that the Cambridge Analytica data scandal (where the personal data belonging to millions of Facebook users was collected without their consent) occurred in March 2018 (during Wave 6). Perhaps the increase in media attention regarding social media privacy policies spurred Facebook to change their algorithms, shift their estimates, or change how data is shared in the ADs manager (i.e. bottom-coding, rounding).
...
Here, we use FB data to estimate state level population by waves. We specifically focus on Washington. First, we estimate migrant population in Washington according to the FB data Then, we estimate migration population in Washington according to origin.  

When estimating migrant population in Washington, we find a steady increase from Wave 1 to Wave 5. Then, there is a sharp increase between Wave 5 and Wave 6. This could suggest that the data may not be the most reliable, illustrating a potentially significant barrier in using FB data for research. As a result, we filter out waves 6 and 7 when estimating the migration population in Washington according to origin. While there are slight fluctuations over time, a majority of the migrants are coming from Mexico, India, Vietnam, China, and Canada.The number of migrants coming from Mexico to Washington are significantly greater than those from other countries.  
...
Maps are a great way to visualize migration patterns. Static maps are the most straight-forward way to showcase a map. They do not require a mapping library and can be displayed anywhere images can be displayed.They are not interacting, and one can't zoom or change between the map layers. 

Here, we look at the migration experiences during Wave 4 (this is when Hurricane Maria took place). The maps reveal what we found earlier: that the [estimated population sizes](#expatpopsize) for migrants from Mexico are much larger than those from other countries. To attenuate the difference in the map, the population size is logged. It's also important to note that the FB data only covers 28 countries: it is not the case that there are no migrants from the entire African continent or Latin America. The visualization make this discrepancy more clear.  
...
Unlike static maps, interactive maps allow the user to zoom in and out, identify specific features, or query underlying data or indicator (i.e. socioeconomic status). Here, we created a quick and simple interactive map for exploratory data analysis on the spatial context. 

You can find more detailed instructions on how to create an interactive map at [mapview tutorial](https://bookdown.org/nicohahn/making_maps_with_r5/docs/mapview.html) and by using [cran documentation](https://cran-r-project.org/web/packages/mapview/mapview.pdf). 
...
#### tmap
(Currently debugging - there seems to be an issue with the shapefile)
...
The code provided by Alexander et al on the GitHub repo indicates IPUMS download of the 2017 1-year ACS data with sex, age, bpl and statefip variables. We read in the pre-saved ACS 2017 1 year data here: 
...
Alexander et al note that FB data represents FB users. Thus, it is not representative of the broader population and it is difficult for the findings to be generalizable. However, the ACS is representative. Therefore, the comparison of the FB estimates with the ACS estimates enables us to assess this potential bias. 
---
...
It is important to note that the FB data encapsulates only 28 different countries. When looking at this data, we see that there are differences in the number of estimated migrants when compared to that of the ACS. There are 10 different countries where the FB estimate is greater than the ACS estimate. For the remaining 18 countries, the ACS estimate is larger than the FB estimate. This indicates a misalignment between the data sources. 
...
```
Note: in the table above, positive values suggest a larger ACS estimate and negative values suggest larger FB values. 
...
Difference-in-difference models are panel data methods applied when we are attempting to understand the effect of a particular treatment. Difference-in-difference models are used when some samples are exposed to a treatment and others are not. In this study, Alexander et al used difference-in-difference to estimate the percent change in Puerto Rican migrants in the continental US after Hurricane Maria. They choose this method to overcome issues of non-representativness and fluctuations in the data over time (indpendent of migration events). 
...
The control group includes migrants from all other countries aside from Mexico, which is assumed to follow different migration motivations. Effectively, using this estimator, we are comparing the change in PR migrant population in the US between waves with the change in population for all other migrants (but not Mexican immigrants).


## **National diff-in-diff estimate of out-migration from Puerto Rico**

Using the difference-in-difference model, Alexander et al find a 17% increase int eh number of Puerto Rican migrants present in the continental US between October 2017 and January 2018. 
...
As evident in data, the static map shows the largest increase of Puerto Rican migrants were in Florida. 
...
In terms of who migrated out of Puerto Rico after Hurricane Maria, there is a large percentage increase in the 15-29 year age range. There were less people above the age of 35 migrating out of Puerto Rico in a post-disaster context. 
...
There were greater men than women in all of the nine states: this did not dramatically change after the Hurricane. However, there were some states (i.e. Texas) that suggested an increase of male migrants than other states (i.e. Pennsylvania) that suggested an increase of female migrants. 
...
(actually, we don't have enough information to infer whether individuals originally from PR, were in the US in the last wave, directly returned to PR. it's possible they went to another country. however, it is likely that they would have gone back to pr.)

(Authors note there is some rounding issue after wave 5, thus keeping only those records with `expat_population` over 1000)
...
(again, making assumption that individuals originally from pr are returning to pr)
