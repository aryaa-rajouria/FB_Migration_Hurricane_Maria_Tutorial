...
We now create a chord map to visualize the migration patterns of those in the Facebook dataset.
```{r}
mig_flow <- fb %>% select(orgin, state, expat_population)
...
(estimates fluctuate by wave. notice large jump in estimated expat pop size between waves 5 and 6, 6 and 7. suggests data may not be most reliable. also illustrates a significant barrier in using fb data for research.)
...
Recall the [estimated population sizes](#expatpopsize) for migrants from Mexico are much larger than those from other countries. To attenuate the difference in the map, the population size is logged. (Also: the FB data only covers 28 countries; it is not the case that there are no migrants from the entire African continent or Latin America.)
...
Looking at just the 28 countries for which there is FB data available, we see there are differences in the number of estimated migrants by data source. There are 10 countries where the FB estimate is larger than the ACS estimate, while for the remaining 18 countries the ACS estimate is larger than the FB estimate.
...
might be interesting to ponder as to why we might see such different acs/fb estimates - self-reports? will likely require examining exactly how fb comes up with estimates of the expat population.

to check: if there are distinctive regional patterns.)

[tangent: the large fb underestimate of individuals from china makes some sense, given the great firewall and fb being banned there]
...
Table 1 in the paper, and also a visual look at the difference in sex ratios by data source for selected states
...
(The first paragraph in the results section of the paper)
