...
When the data is disaggregated as per the different waves, we find that there are differences in the migration experiences of males and females. In general, there are slightly more female migrants than male migrants. This difference is a little more accentuated in Wave 1, Wave 3, Wave 5 and Wave 6. 

```{r}
#looking at how many people who migrated were male vs female 
sex_comparison <- fb %>% group_by(sex, wave, date) %>% summarise(n = sum(expat_population)) %>% mutate(sex = ifelse(sex == 1, "male", "female")) 

kable(sex_comparison %>% pivot_wider(names_from=sex, values_from=n), 
...
#age_wave_comparison <- fb %>% group_by(wave, age_group) %>% summarise(n = sum(expat_population))

ggplot(data=age_comparison, aes(x=wave, y=n/1e6, fill=factor(age_group))) +
  geom_bar(position="dodge", stat="identity") +
  labs(y="count in millions", fill="age") +
